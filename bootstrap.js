...
FaviconService = FaviconService && FaviconService.getService(Ci.nsIFaviconService).QueryInterface(Ci.mozIAsyncFavicons);
...
let i$ = {
	...
	getLink: function (win, link, base)
	{
		...
		let clean = bro.cleanLink(link, base);
		LOG([link, clean]);
		return (clean != link) ? (bro.blink(win), clean) : null;
	},
			...
			case 'nsPref:changed':
				switch (d)
				{
					...
					case 'httpomr':
						...
						if (d === 'enabled')
						{
							this.wmForeach(this[(v ? 'AttachDOMLoad' : 'DetachDOMLoad')]);
						}
					default:
						break;
				}
			...
			case 'http-on-modify-request':
				{
					...
					if (s.loadFlags & s.LOAD_DOCUMENT_URI)
					{
						...
						if (win)
						{
							...
							if (link && link != s.originalURI.spec)
							{
								try
								{
									...
									if (dom === c.URI.host || dom !== uri.host)
									{
										...
										LOG('^ Skipping redirect: ' + c.URI.spec + '  ->  ' + link);
								}
								catch (e)
								{
									Cu.reportError(e);
...
(function (global) global.loadSubScript = function (file, scope) Services.scriptloader.loadSubScript(file, scope || global))(this);
...
function loadIntoWindow(window)
{
	...
	function create(node) window.document.createElement(node);

	function $(id) window.document.getElementById(id);

	function e(nodeName, attributes, children, parentNode)
	{
		let node = create(nodeName);
		if (!node) return null;
				...
				case 2:
					{
						...
						let r = Math.min(14, Math.max(Object.keys(cltrack).length, 8)),
							wpr = 30;
						...
						for (let l in cltrack)
						{
							try
							{
								...
								item = e('listitem', { allowevents: true, maxheight: 18 }, [c1, c2], t);
	...
	e('tooltip', { id: addon.tag + '-tooltip' }, 0, $('mainPopupSet'))
		.addEventListener('popupshowing', wmsData.popupshowing = ev => window.diegocr[addon.tag].load(ev), true);

	e('panel',
		{
			id: addon.tag + '-context',
			backdrag: 'true',
	...
	tn.addEventListener('click', wmsData.TBBHandler, false);
...
function unloadFromWindow(window)
{
	...
	let btnId = addon.tag + '-toolbar-button',
		btn = $(btnId);
	...
	if (addon.wms.has(window))
	{
		...
		if (wmsData.popupshowing)
		{
			...
			if (tt) tt.removeEventListener('popupshowing', wmsData.popupshowing, false);
		}
		...
		if (!addon.branch.getPrefType(k) || Reset)
		{
			switch (typeof v)
			{
				case 'boolean':
					...
					break;
				case 'number':
					addon.branch.setIntPref(k, v);
					break;
				case 'string':
					addon.branch.setCharPref(k, v);
					break;
			}
		}
	}
...
function startup(data)
{
	...
	tmp.AddonManager.getAddonByID(data.id, function (data)
	{
		...
		['enabled', 'progltr', 'httpomr', 'cltrack'].forEach(function (p) addon[p] = addon.branch.getBoolPref(p));
		i$.addHTTPObserver();
		i$.addHTTPObserver2();
